...
#tokenize text into words and stem each word to get the root word
def stem(texts):
...
def get_cluster_labels(texts: Sequence[str], n_clusters: int) -> Sequence[int]:
    ...
    # Get the shape of 'embedding_vectors' and 'centroids'
    embedding_shape = embedding_vectors.shape
    centroids_shape = centroids.shape
    centroids = tf.convert_to_tensor(centroids, dtype=tf.dtypes.float32)
    embedding_vectors = tf.convert_to_tensor(embedding_vectors.todense(), dtype=tf.dtypes.float32)
    softmaxed_distances_list = []
    # Loop through each text in 'embedding_vectors'
    for i in range(embedding_vectors.shape[0]):
        # Get the current text
        text = embedding_vectors[i:i+1]  # Shape: (1, 10000)

        # Compute the distances for the current text
        distances = tf.norm(text - centroids, axis=-1)  # Shape: (1, 50)
        distances = tf.reshape(distances, [1, distances.shape[0]])

        # Compute softmax on the distances
        softmaxed_distances = tf.nn.softmax(distances, axis=-1)  # Shape: (1, 50)
        # Append the softmaxed distances to the list
        softmaxed_distances_list.append(softmaxed_distances)
    # Stack the computed softmaxed distances to obtain the final tensor
    softmaxed_distances_tensor = tf.concat(softmaxed_distances_list, axis=0)  # Shape: (14999, 50)
    if args.soft_label:
        return softmaxed_distances_tensor
    else:
...
def train(texts, labels, is_cluster_labels, keep_classification_layer=True, model_name_or_path="bert-base-uncased",
    ...
    if args.soft_label and is_cluster_labels:
        #print("soft label", labels.shape)
        num_labels = labels.shape[1]
    else:
        num_labels = len(set(labels))
        #print("hard label", num_labels)
...
if __name__ == '__main__':
    ...
    inter_trained_model_path = train(unlabeled_texts, clustering_pseudo_labels, is_cluster_labels = True, keep_classification_layer=False,
    ...
    model_finetuned_over_base_path = \
        train(sample_texts, sample_labels, is_cluster_labels=False, keep_classification_layer=True, num_epochs=args.finetuning_epochs)
